...
async function chatSyncWorker(args: {
  ...
  while (true) {
    ...
    if (messageBlob !== undefined) {
      formData.append('messages', new Blob([messageBlob.buffer.slice(messageBlob.byteOffset, messageBlob.byteOffset + messageBlob.byteLength)]));
    }
    if (snapshotBlob !== undefined) {
      formData.append('snapshot', new Blob([snapshotBlob.buffer.slice(snapshotBlob.byteOffset, snapshotBlob.byteOffset + snapshotBlob.byteLength)]));
    }
